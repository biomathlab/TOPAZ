...
def run_save_crocker(args):
    ...
    DF_PATH = './Simulated_Grid/ODE_Align/Cidx_'+str(C_idx).zfill(2)+'_Lidx_'+str(L_idx).zfill(2)+'_Widx_'+str(W_idx).zfill(2)+'/run_1/df.pkl'
    ...
    if 'vx' in DATA_COLS:
        SAVE_PATH = './Simulated_Grid/ODE_Align/Cidx_'+str(C_idx+1).zfill(2)+'_Lidx_'+str(L_idx+1).zfill(2)+'_Widx_'+str(W_idx).zfill(2)+'/run_1/crocker_velocities.npy'
    if 'angle' in DATA_COLS:
        SAVE_PATH = './Simulated_Grid/ODE_Align/Cidx_'+str(C_idx+1).zfill(2)+'_Lidx_'+str(L_idx+1).zfill(2)+'_Widx_'+str(W_idx).zfill(2)+'/run_1/crocker_angles.npy'
...
Cs = np.linspace(0.1,3.0,11)
Ls = np.linspace(0.1,3.0,11)
...
for C_idx, C in enumerate(Cs):
    for L_idx, L in enumerate(Ls):
        for W_idx, W in enumerate(Ws):
            list_tuples.append((C_idx, L_idx, W_idx, DATA_COLS, true_FRAME_LIST, pred_FRAME_LIST, PROX_VEC))
