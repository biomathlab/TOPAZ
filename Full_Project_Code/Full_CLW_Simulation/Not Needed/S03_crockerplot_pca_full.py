...
for W_idx, W in enumerate(Ws):
    ...
    for irun in range(num_run):
        ...
        np.save('./Simulated_Grid/ODE_Align/PCA_TSNE/pca_results_full'+str(irun+1)+'_angles_Widx_'+str(W_idx).zfill(2)+'.npy',results)
    print("Job done!")
    
    
    tsne_results = np.load('./Simulated_Grid/ODE_Align/PCA_TSNE/pca_results_full'+str(irun+1)+'_angles_Widx_'+str(W_idx).zfill(2)+'.npy',allow_pickle=True).item()
    ...
    fig.tight_layout()
    fig.savefig('./Simulated_Grid/ODE_Align/PCA_TSNE/pca_crocker_heatmap_angles_Widx_'+str(W_idx).zfill(2)+'.pdf',bbox_inches='tight')
    plt.show()
    ...
    scipy.io.savemat('./Simulated_Grid/ODE_Align/PCA_TSNE/pca_mat_angles_SD_Widx_'+str(W_idx).zfill(2)+'.mat',mat_tsne)
